\begin{algorithm}
  ...
  \Fn{\F{$\mathcal{K}$, $questions$, $steps$, $B$, $T$, $lr$, $\beta$, $\gamma$, $\lambda$}}{%
    ...
    \For{\textbf{each} $q$ \textbf{in} $questions$}{%
      Link the cause and effect of $q$ to entities $e_c$, $e_e$ in $\mathcal{K}$\;
    ...
    \While{step < steps}{%
      ...
      \For{$t = 0$ \textbf{to} $T-1$}{%
        ...
        Receive state $s_{t+1} = \delta (s_t, a_t)$ and reward $r_t = \mathcal{R}(s_{t}, a_{t})$\; \label{ln:20}
      ...
      \If{$|path\_rollouts| = B$}{%
        ...
        \For{\textbf{each} $((s_0, a_0, r_0, \mathcal{A}_0^{\psi}, R_0(\lambda)),\dots,$ \\ $(s_{T-1}, a_{T-1}, r_{T-1}, \mathcal{A}_{T-1}^{\psi}, R_{T-1}(\lambda)))$ \textbf{in} path\_rollouts}{%
          $policy\_update \mathrel{+} = \sum_{t=0}^{T-1} \nabla_{\theta} \log \pi_{\theta}(a_t | s_t) \ \mathcal{A}_t^{\psi}$\;
          $value\_update \mathrel{+} = \sum_{t=0}^{T-1} \nabla_{\psi} (R_t(\lambda) - V_{\psi}(s_t))^2$\;
        }
