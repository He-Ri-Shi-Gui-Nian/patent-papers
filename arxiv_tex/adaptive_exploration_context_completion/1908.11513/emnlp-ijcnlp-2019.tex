...
\maketitle
\begin{abstract}
%   multi-hop reasoning is an effective method for predicting the target entity with reasoning paths in query answering (QA) task. 
%   Compared with knowledge graph embedding methods, it can also give reasoning paths to make inference process explainable. 
%   Most previous models assume that every relation in knowledge graph has enough triples for training. However, there are still many few-shot relations in knowledge graph and the performance of previous work on these relations will drop significantly. In this paper, we propose a meta-learning based model named Meta-KGR to learn good meta parameters from high-frequency relations, which can be used as well-initialized parameters for few-shot relations. We evaluate our model on two datasets from Freebase and NELL and the experimental results show that Meta-KGR outperforms state-of-the-art methods.
...
% Theoretically, we can search the target entity from KGs as the answer.
...
% Large scale knowledge graphs (KGs) have proven to be helpful for many NLP tasks such as query answering (QA) and semantic search in recent years, which can be formalized as a triple query $(e_s, r_q, ?)$, where $e_s$ is the source entity and $r_q$ is the query relation. For example, given a language query "\textit{Who is the wife of Bill Gates?}", which can transformed into $(\textit{Bill Gates}, \textit{wife}, ?)$, we can go over knowledge graphs to search the target entity. However, many KGs are constructed automatically and face serious incompleteness problems\cite{TransE}, which causes that we may not get the target entity directly.

% In recent years, knowledge graph embedding methods \cite{TransE, ConvE}, which encode entities and relations into a semantic space, have shown strong abilities to predicting the target entity for an triple query. Different from knowledge reasoning models, these embedding based approaches only give answers and lack interpretability for their decisions \cite{MultiHop}. In order to address these problems, \citeauthor{MINERVA} and \citeauthor{MultiHop} propose multi-hop reasoning models, which leverage the symbolic compositionality of relations in knowledge graphs to make the reasoning explainable. For example, when predicting the target entity of $(\textit{Trump}, \textit{nationality}, ?)$, multi-hop reasoning models can give not only the target entity \textit{America} but also multi-hop explainable paths $(\textit{Trump}, \textit{bornIn}, \textit{New York}) \wedge (\textit{New York}, \textit{locatedIn}, \textit{America})$.

% Most previous work assumes that there are enough triples for every relation in KGs. However, a large portion of KG relations are actually long-tail \cite{one-shot}, which can be called few-shot relations. Experimental results show that the performance of previous multi-hop reasoning models like MINERVA\cite{MINERVA} and MultiHop\cite{MultiHop} on these few-shot relations will drop significantly. There are some knowledge graph embedding models \cite{one-shot, shi2018open, DKRL} that can deal with zero-shot or one-shot relations, but still have two main drawbacks: (1) they are embedding-based models and lack interpretability; (2) they focus on zero- or one-shot relatiosn and can not handle few-shot relations very well.
% % limited application scenarios. Compared with one-shot and few-shot relations, the real KGs have the largest number of few-shot relations \cite{one-shot}. 
% Therefore, few-shot knowldge graph reasoning is a quite important problem that has not yet been fully resolved.

% In this paper, we propose a meta-learning algorithm for multi-hop reasoning to address problems above. Specifically, we regard triple queries with the same relation $r$ in KGs as a task and learn a REINFORCE model for every task respectively to search target entities and reasoning paths. Based on previous meta-learning method MAML \cite{MAML}, we use tasks of high-frequency relations to learn a meta model.
% % We construct our model based on the famous meta-learning method MAML \cite{MAML}, which tries to learn a meta model between different tasks. 
% % Specifically, we regard every relation in KGs as a task and use high-frequency relations to learn a meta model.
% % which can be used as well-initialized parameters for few-shot relations. 
% The learned meta infomation includes common features between different tasks and provide a good starting point, from which we can easily learn a REINFORCE model for every task of few-shot relation.
% Experimental results show that our model outperforms previous models in few-shot relations from Freebase and NELL.
...
% which makes the training more efficient and encourages the policy to sample more diverse paths. 
...
% \textbf{Knowledge Graph Embedding.} Knowledge graph embedding \cite{TransE, DistMult, ConvE} aims to learn vectors of entities and relations in low-dimension space, which can be used to define a score function $f(e_s, r, e_o)$ to evaluate the probability that a triple is true. In recent years, several models \cite{DKRL, shi2018open} use additional entity descriptions to handle unseen entities, which can be seen as zero-shot entity learning. GMatching \cite{one-shot} predicts new facts under a challenging setting where only one training triple for a given relation $r$ is available. It can be seen as one-shot relational learning for knowledge graphs.

% \textbf{Multi-Hop Reasoning.}  Multi-Hop reasoning aims to learn symbolic inference rules from relational paths in $\mathcal{KG}$ and has been formulated as sequential decision problems in recent years. DeepPath \cite{DeepPath} and DIVA \cite{chen2018variational} uses REINFORCE \cite{RL} to search for paths in knowledge graph. MINERVA \cite{MINERVA} first uses RL to search the target entity from knowledge graph for a given query end-to-end. Reinforce-Walk \cite{shen2018reinforcewalk} further proposes to solve the reward sparsity problem in walk-based QA using off-policy learning. MultiHop \cite{MultiHop} adds reward shaping and action drop for MINERVA to make the training more efficient and encourage the policy to sample diverse paths. MultiHop also achieves state-of-the-art performance.

% \textbf{meta-learning.} meta-learning tries to solve the problem of ``fast adaptation on a new training task''. It is proved to be very successful on few-shot (or one-shot) task \cite{lake2015human, meta-translate}. Previous meta-learning mainly focus on computer vision and imitation learning domains. In this paper, we propose a new model named Meta-KGR using the meta-learning algorithm MAML \cite{MAML} for few-shot multi-hop reasoning. To the best of our knowledge, this work is the first research on few-shot learning for multi-hop reasoning.
...
% because we unroll the search up to a fixed number of time steps $T$.
...
% Given query relation $r$, our relation-specific policy network is trained to maximizes the expected reward:
% \begin{equation}
%     J_r(\theta) = \mathbb{E}_{(e_s, r, e_o) \in \mathcal{KG} } \mathbb{E}_{a_1, \dots, a_{T-1} \in \pi_{\theta} } [R(s_T|e_s, r)].
% \end{equation}

% To solve this optimization problem, we employ REINFORCE \cite{RL} with the following stochastic gradient:
% \begin{equation}
%     \nabla_{\theta} J_r(\theta) \approx \nabla_{\theta} \sum_{t} R(s_T|e_s, r)\text{log}\pi_{\theta}(a_t|s_t)
% \end{equation}

\subsection{Meta-Learning}

% In few-shot multi-hop reasoning, meta-learning aims to learn a well-initialized parameters $\theta^*$ for relation-specific learning of few-shot query relations. Specifically, every query relation in knowledge graph is regarded as a task $\mathcal{T}_i$.

% For every normal relation $r$ in knowledge graph, we use the randomly initialized parameters to learn a relation-specific policy network $\pi_{\theta_r}$. 
