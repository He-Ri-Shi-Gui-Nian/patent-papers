...
\midrule
\rotatebox[origin=c]{90}{\tiny{IF}} & IFEval & 20.8 \tiny{$\pm 1.7$} & 44.7 \tiny{$\pm 4.9$} & 0.25\tiny{$\pm 0.02$}\\
\bottomrule
\end{tabular}
\caption{Statistics showing the percentage of \textbf{Drifting Samples} at any point in the debate, the percentage of \textbf{Recovering Samples} relative to the drifting ones, and the average number of turns after which problem drift occurs \textbf{$FOCUS_r \neq 0$}. The highest and lowest values per column are displayed in \textbf{bold}. Values are reported with their respective standard deviations for three runs. MAD uses \textit{Llama-3.1} and Voting.\vspace{-0.5cm}}
